...
#Histograms of Poverty Rate by State
...
plot1 <- ggplot(mp_train, aes(x=secc_pov_rate_rural)) + 
  ...
  ylab("Count") +
  labs(title = "Rural Poverty Distribution, Madhya Pradesh") +
  theme_minimal() + 
  theme(
...
plot2 <- ggplot(as_train, aes(x=secc_pov_rate_rural)) + 
  ...
  xlab("% of Shrid Households in Poverty") +
  ylab("Count") +
  labs(title = "Rural Poverty Distribution, Assam") +
  theme_minimal() + 
  theme(
    axis.title = element_text(size = 8), 
    axis.text = element_text(size = 8),
...
plot3 <- mp_train %>%
  ...
  labs(title = "Shrid Poverty Rate vs. Night Light, Madhya Pradesh",  
       xlab("Average Night Light Radiance (nW/cm^2/sr)"),
       ylab("% of Shrid Households in Poverty")) +
  theme_minimal() + 
  theme(
    axis.title = element_text(size = 8), 
    axis.text = element_text(size = 8),
...
plot4 <- as_train %>%
   ggplot(aes(x = viirs_annual_mean, y = secc_pov_rate_rural)) + 
  geom_point(alpha = 0.25) +
  labs(title = "Shrid Poverty Rate vs. Night Light, Assam",  
       xlab("Average Night Light Radiance (nW/cm^2/sr)"),
       ylab("% of Shrid Households in Poverty")) +
  theme_minimal() + 
  theme(
...
#Poverty Rate vs. Middle School Education 
...
plot6 <- as_train %>%
  ggplot(aes(x = ed_mid_share, y = secc_pov_rate_rural)) +
  geom_smooth(alpha = 0.5) +
  labs(title = "Level of Education vs. Poverty rate, Assam", 
       subtitle = "(Source: Socioeconomic & Caste Census 2011-2012)") +
  xlab("Share of Rural Population with Middle School Education") +
  ylab("Percent of Rural Households Below Poverty Line") +
  theme_minimal()
...
#Poverty Rate by Proportion of Population that is SC/ST
...
labs(
  x = "Proportion of Scheduled Caste/Tribe Proportion (Shrid)",
  y = "Percent of Rural Households Below Poverty Line",
  title = "Poverty Rate by Schedule Caste/Tribe Population Proportion, Assam",
  subtitle = "(Source: 2011 Population Census Abstract, Socioeconomic & Caste Census 2011)"
)
...
labs(
  x = "Literate Proportion of Population",
  y = "Poverty Rate",
  title = "Poverty Rate by Literacy Rate, Assam",
  subtitle = "(Source: 2011 Population Census Abstract, Socioeconomic & Caste Census 2011)"
)
...
#Poverty Rate by Proportion of Agricultural Employment
...
labs(
  x = "Proportion of Workers who are Cultivators",
  y = "Poverty Rate",
  title = "Poverty Rate by Agricultural Employment, Assam",
  subtitle = "((Source: 2011 Population Census Abstract, Socioeconomic & Caste Census 2011)"
)
...
#Assam: geospatial analysis [Jacob to edit]
```{r}
...
#Merging sf file into madhya_pradesh
#assam_sf <- merge(assam_v2, shrid, by = c("shrid2.x"))

#Converting data
#assam_sf <- st_as_sf(assam_sf)
...
shrid <- st_read("shrid2_open.shp", quiet = TRUE)
shrid <- st_transform(shrid, crs = 4326)
shrid_v2 <- readOGR("shrid2_open.shp")
```
...
names(assam_v3)[names(assam_v3) == 'shrid2.x'] <- 'shrid2'
```

```{r}
assam_merged <- merge(assam_v3, shrid, by = c("shrid2"))
assam_map_setup <- merge(shrid_v2, assam_merged, by = c("shrid2"))

#Converting data
assam_map_setup <- st_as_sf(rural_map_setup)

#Removing missing data from poverty rate column
assam_map_setup <- assam_map_setup %>%
  filter(!is.na(secc_pov_rate_rural))

#Continuing to transform data
#into form in which we can map into
assam_map_setup <- st_transform(rural_map_setup, crs = 4326)
```

```{r}
#Creating map of rural poverty
assam_map_setup %>% 
  ggplot() +
  geom_sf(color = NA, aes(fill = secc_pov_rate_rural)) +
  labs(title = "Mapping poverty in rural shrids of Assam",
...
as_train <-
  as_train %>%
  select(-c(year, shrid2.x, shrid2.x.x, shrid2.y, shrid2.y.y, TTTTTT))
```
...
as_rec <- recipe(formula = secc_pov_rate_rural ~ ., 
                 data = as_train) %>%
...
```
Assam Model 2: Linear Regression 
```{r}
#create model object
lm_mod <- linear_reg() %>%
  set_engine("lm")
...
Jacob's test involving classification

```{r}
#50%+ of rural households in poverty
...
cart_mod_classification <-
  decision_tree() %>%
  set_engine(engine = "rpart") %>%
  set_mode(mode = "classification")
cart_wf_classification <- workflow() %>%
  add_recipe(cart_rec) %>%
  add_model(cart_mod_classification)
# fit the model
cart_fit_classification <- 
  cart_wf_classification %>%
  fit(data = as_train)
```

```{r}
rpart.plot::rpart.plot(x = cart_fit_classification$fit$fit$fit)
```
...
predictions <- bind_cols(
  as_test,
  predict(object = cart_fit_classification, new_data = as_test),
  predict(object = cart_fit_classification, new_data = as_test, type = "prob")
)
select(predictions, high_poverty, starts_with(".pred"))
```
```{r}
library(yardstick)
...
estimate = .pred_class)
```
(842+3882)/(842+3882+632+1362)

Accuracy: 70%

```{r}
precision_vec(data = predictions,
truth = predictions$high_poverty,
