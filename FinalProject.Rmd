...
#Assam: data visualization w/ training dataset
```{r - Data Visualization (Assam)}
#Graph 1: Histogram of Poverty Rate (Assam)
ggplot(as_train, aes(x=secc_pov_rate_rural)) + 
  geom_histogram() +
  geom_vline(xintercept = mean(as_train$secc_pov_rate_rural, na.rm = TRUE), col = "red") +
  xlab("Percent of Rural Households Below Poverty Rate") +
  ylab("Count") +
  labs(title = "Rural Poverty Rate Distribution, Assam (Shrid-Level)")

#Graph 2: Poverty Rate vs Night Lights (Assam)
as_train %>%
  ggplot(aes(x = viirs_annual_mean, y = secc_pov_rate_rural)) +
  geom_point(alpha = 0.25) +
  labs(title = "Poverty rate vs. Night Light, Assam", 
       subtitle = "(Source: Socioeconomic & Caste Census 2011, VIIRS Night Lights 2012)") +
  xlab("Average Annual Night Light in Shrid") +
  ylab("Percent of Rural Households Below Poverty Line") +
  theme_minimal()

#Graph 3: Poverty Rate vs. Middle School Education (Assam)
as_train %>%
  ggplot(aes(x = ed_mid_share, y = secc_pov_rate_rural)) +
  geom_smooth(alpha = 0.5) +
...
mp_rec <- recipe(formula = secc_pov_rate_rural ~ ., 
                 data = mp_train) %>%
  step_zv(all_predictors()) %>%
  step_select(all_numeric()) %>%
  step_center(all_predictors()) %>%
